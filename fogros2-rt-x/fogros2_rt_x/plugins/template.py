...
class FanucDatasetConfig(BaseDatasetConfig):
    def __init__(self):
        ...
        OBSERVATION_SPEC = [
            FeatureSpec("an_image", Image(shape=(224, 224, 3), dtype=tf.uint8)),
            FeatureSpec("a_tensor_as_observation", Tensor(shape=(7,), dtype=tf.float32)),
        ]

        ACTION_SPEC = [
            FeatureSpec("a_tensor_as_action", Tensor(shape=(6,), dtype=tf.float32)),
        ]
